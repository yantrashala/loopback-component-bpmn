...
function getExecuteOption(engine) {
...
Engine.taskHandler = function (app, task) {
  debug('received task completion event');
  Instance.findOne({ where: { id: task.formDef.instanceID } })
    .then((instance) => {
      if (!instance) throw new Error('Instance not found')

      instance.updateAttribute('status', 'running');
      ...
      engine.resume(instance.state, executeOptions, function (err, definition) {
        if (err) return onEngineError(engine, err)
        debug('resume finished');
      });
      debug('resuming execution for instance id:', instance.id);
      process.nextTick(function () { engine.signal(task.name) });
    });
}
...
function onEngineError(engine, err, eventSource) {
  Instance.findOne({
    ...
    .then((instance) => {
      return instance.updateAttribute({
        status: 'error',
        errorData: {
          'err': err,
...
function onEngineEnd(engine, err, definition) {
  ...
  return Instance.findOne({
    ...
    .then((instance) => {
      ...
      return instance.updateAttribute('status', status);
    })
    .finally(() => {
      engine = null;
    });
